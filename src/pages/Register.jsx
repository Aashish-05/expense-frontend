...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error aaya, try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <p style={styles.link}>Already registered? <Link to="/login">Login karein</Link></p>
      </div>
    </div>
  )
}
...
const styles = {
  ...
  error: { color: 'red', marginBottom: '1rem', fontSize: '0.85rem' },
